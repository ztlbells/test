...
func (t *SimpleChaincode) issueCoinToBank(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	transaction := Transaction{FromType:0,FromID:0,ToType:1,ToID:args[0],Time:time.Now().Unix(),Number:issueNumber,ID:transactionNo}
	err = writeTransaction(stub,transaction)
	if err != nil {
		return nil, errors.New("write Error" + err.Error())
	}
...
func (t *SimpleChaincode) issueCoinToCp(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	transaction := Transaction{FromType:1,FromID:strconv.Atoi(args[0]),ToType:1,ToID:args[0],Time:time.Now().Unix(),Number:issueNumber,ID:transactionNo}
	err = writeTransaction(stub,transaction)
	if err != nil {
		return nil, errors.New("write Error" + err.Error())
	}
...
func (t *SimpleChaincode) transfer(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	cpFrom.Nubmer = cpFrom.Nubmer - issueNumber
	...
	transaction := Transaction{FromType:2,FromID:cpFromId,ToType:2,ToID:cpToId,Time:time.Now().Unix(),Number:issueNumber,ID:transactionNo}
	err = writeTransaction(stub,transaction)
	if err != nil {
		return nil, errors.New("write Error" + err.Error())
	}
